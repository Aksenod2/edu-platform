...
describe('sweepAutoDoneSessions — авто-перевод в done по дате', () => {
  it('выбирает только planned-занятия с датой в прошлом (в окне просмотра)', async () => {
  ...
  it('завершает занятие, которое НЕ трогали после его даты (updatedAt в пределах grace)', async () => {
    ...
    expect(count).toBe(1);
    // updateMany с повторным условием status='planned' (атомарно/идемпотентно).
    expect(db.session.updateMany).toHaveBeenCalledWith({
      where: { id: 'sess-1', status: 'planned' },
      data: { status: 'done' },
    });
  });
