...
export async function paymentRoutes(app: FastifyInstance) {
  ...
  app.post('/topup-requests', { onRequest: authenticate }, async (request, reply) => {
    ...
    if (rawClaimed !== undefined && rawClaimed !== '') {
      ...
      if (!isPositiveInt(parsed)) {
        return reply
          .status(400)
          .send({ error: 'Заявленная сумма должна быть положительным целым числом копеек' });
      }
  ...
  app.post('/admin/topup-requests/:id/approve', { onRequest: adminOnly }, async (request, reply) => {
    ...
    if (!isPositiveInt(body.amountKopecks)) {
      return reply
        .status(400)
        .send({ error: 'Сумма должна быть положительным целым числом копеек' });
    }
  ...
  app.put('/admin/payment-settings', { onRequest: adminOnly }, async (request) => {
