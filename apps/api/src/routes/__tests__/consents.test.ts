...
beforeEach(() => {
  ...
  // «записей и версий нет». Конкретные тесты переопределяют через mockResolvedValueOnce.
...
describe('POST /public/streams/join/:token — согласия при регистрации', () => {
  ...
  it('без body.consents согласия не пишутся (поведение как раньше)', async () => {
    ...
    expect(res.statusCode).toBe(201);
    // legalDocumentVersion.findMany при этом ВЫЗЫВАЕТСЯ — pendingRequiredConsents
...
// Опубликованные версии документов всех обязательных типов
// (offer → offer, personalData/serviceNotifications → pd-consent).
const publishedRequiredVersions = [
  { document: { slug: 'offer' } },
  { document: { slug: 'pd-consent' } },
];
...
describe('pendingRequiredConsents — недостающие обязательные согласия', () => {
  it('нет granted-записей, версии опубликованы → все обязательные типы', async () => {
    ...
    db.legalDocumentVersion.findMany.mockResolvedValueOnce(publishedRequiredVersions);
  ...
  it('все обязательные granted → пусто (версии даже не запрашиваются)', async () => {
    db.userConsent.findMany.mockResolvedValueOnce(
